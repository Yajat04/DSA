class Solution {
    ...
    void solve(int idx, int m, vector <int> &people_skills, vector <int> temp, int mask){
        if(idx == m){
            if(mask == target_mask && 
                (result.size() == 0 || result.size() > temp.size())) result = temp;
            return;
        }
        ...
        //take
        if(people_skills[idx] != 0){ //or mask | people_skills[idx] != mask , means p[idx] isnt empty and have some skills o/w it wont make any sense to take  it
            temp.push_back(idx);
